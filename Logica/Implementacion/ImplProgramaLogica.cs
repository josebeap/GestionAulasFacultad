...
{
    public class ImplProgramaLogica
    {
        private ImplProgramaDatos accesoDatos;
        public ImplProgramaLogica()
        {
            this.accesoDatos = new ImplProgramaDatos();
        }
        ...
        public IEnumerable<ProgramaDTO> ListarRegistros(String filtro, int numPagina, int registrosPorPagina, out int totalRegistros)
        {
            var listado = this.accesoDatos.ListarRegistros(filtro, numPagina, registrosPorPagina, out totalRegistros);
            MapeadorProgramaLogica mapeador = new MapeadorProgramaLogica();
            return mapeador.MapearTipo1Tipo2(listado);
        }
        // esto va en producto
        public IEnumerable<ProgramaDTO> ListarRegistros()
        {
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public ProgramaDTO BuscarRegistro(int id)
        {
        ...
        public Boolean EditarRegistro(ProgramaDTO registro)
        {
            MapeadorProgramaLogica mapeador = new MapeadorProgramaLogica();
            ProgramaDbModel reg = mapeador.MapearTipo2Tipo1(registro);
            Boolean res = this.accesoDatos.EditarRegistro(reg);
            return res;
        }

        public Boolean GuardarRegistro(ProgramaDTO registro)
        {
            MapeadorProgramaLogica mapeador = new MapeadorProgramaLogica();
            ProgramaDbModel reg = mapeador.MapearTipo2Tipo1(registro);

...
{
    public class ImplReservaLogica
    {
        private ImplReservaDatos accesoDatos;
        public ImplReservaLogica()
        {
            this.accesoDatos = new ImplReservaDatos();
        }
        ...
        public IEnumerable<ReservaDTO> ListarRegistros(String filtro, int numPagina, int registrosPorPagina, out int totalRegistros)
        {
            var listado = this.accesoDatos.ListarRegistros(filtro, numPagina, registrosPorPagina, out totalRegistros);
            MapeadorReservaLogica mapeador = new MapeadorReservaLogica();
            return mapeador.MapearTipo1Tipo2(listado);
        }
        // esto va en producto
        public IEnumerable<ReservaDTO> ListarRegistros()
        {
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public ReservaDTO BuscarRegistro(int id)
        {
        ...
        public Boolean EditarRegistro(ReservaDTO registro)
        {
            MapeadorReservaLogica mapeador = new MapeadorReservaLogica();
            ReservaDbModel reg = mapeador.MapearTipo2Tipo1(registro);
            Boolean res = this.accesoDatos.EditarRegistro(reg);
            return res;
        }

        public Boolean GuardarRegistro(ReservaDTO registro)
        {
            MapeadorReservaLogica mapeador = new MapeadorReservaLogica();
            ReservaDbModel reg = mapeador.MapearTipo2Tipo1(registro);

...
{ 
	...
	{
		public IEnumerable<tb_auxiliar> ListarRegistros(String filtro)
        {
			var lista = new List<tb_auxiliar>();
            var listaPersonas = new List<tb_persona>();
            using (SoftwareBDEntities bd = new SoftwareBDEntities())
            {
                if (String.IsNullOrWhiteSpace(filtro))
                {
                    lista = bd.tb_auxiliar.ToList();
                }
                else
                {   
                    ...
                    {
                        lista.Add((tb_auxiliar)bd.tb_auxiliar.Where(x => x.id_persona.Equals(i.id)));
                    }
        ...
        public bool GuardarRegistro(tb_auxiliar registro)
        {
            try
            {
                using (SoftwareBDEntities bd = new SoftwareBDEntities())
                {
                    ...
                    if (bd.tb_auxiliar.Where(x => x.id_persona.Equals(registro.id_persona)).Count() > 0)
                    {
                        return false;
                    }
                   
                    
                    bd.tb_auxiliar.Add(registro);
                    bd.SaveChanges();
                    return true;
                }
            }
        ...
        public tb_auxiliar BuscarRegistro(int id)
        {
            using (SoftwareBDEntities bd = new SoftwareBDEntities())
            {
        ...
        public bool EditarRegistro(tb_auxiliar registro)
        {
            try
            {
                using (SoftwareBDEntities bd = new SoftwareBDEntities())

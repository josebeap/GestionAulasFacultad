...
{ 
	...
	{
		public IEnumerable<tb_monitor> ListarRegistros(String filtro)
        {
        ...
        public bool GuardarRegistro(tb_monitor registro)
        {
            try
            {
                using (SoftwareBDEntities bd = new SoftwareBDEntities())
                {
                    ...
                    if (bd.tb_monitor.Where(x => x.id_persona.Equals(registro.id_persona)).Count() > 0)
                    {
                        return false;
                    }
                   
                    
                    bd.tb_monitor.Add(registro);
                    bd.SaveChanges();
                    return true;
                }
            }
        ...
        public tb_monitor BuscarRegistro(int id)
        {
            using (SoftwareBDEntities bd = new SoftwareBDEntities())
            {
        ...
        public bool EditarRegistro(tb_monitor registro)
        {
            try
            {
                using (SoftwareBDEntities bd = new SoftwareBDEntities())
